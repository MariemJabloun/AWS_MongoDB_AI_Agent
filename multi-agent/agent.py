...
def place_lookup_by_country(query_str: str) -> str:
    ...
    """
    logger.info("Looking up places by country: %s", query_str)
    try:
        ...
        logger.info("Found %d places in country: %s", len(places), query_str)
        return str(places)
    except Exception as e:
        logger.error("Error looking up places by country '%s': %s", query_str, e)
        raise
...
def place_lookup_by_name(query_str: str) -> str:
    ...
    """
    logger.info("Looking up place by name: %s", query_str)
    try:
        client = get_mongo_client()
        collection = get_travel_collection(client)
        res = ""
        ...
        res = collection.find_one(filter=filter_in, projection=project)
        logger.info("Found place details for: %s", query_str)
        return str(res)
    except Exception as e:
        logger.error("Error looking up place by name '%s': %s", query_str, e)
        raise
...
def place_best_time_lookup(query_str: str) -> str:
    ...
    """
    logger.info("Looking up best time to visit for place: %s", query_str)
    try:
        client = get_mongo_client()
        collection = get_travel_collection(client)
        res = ""
        ...
        res = collection.find_one(filter=filter_in, projection=project)
        logger.info("Found best time to visit for: %s", query_str)
        return str(res)
    except Exception as e:
        logger.error("Error looking up best time to visit for '%s': %s", query_str, e)
        raise
...
def mongodb_search(query: str) -> str:
    ...
    """
    logger.info("Performing semantic search for place features: %s", query)
    try:
        ...
        docs = list(response)
        logger.info("Found %d results from vector search", len(docs))
        ...
        logger.error("Error performing semantic search for query '%s': %s", query, e)
        raise
...
agent = Agent(
    ...
    tools=[
        current_time,
        current_month,
        place_lookup_by_country,
        place_lookup_by_name,
        place_best_time_lookup,
        mongodb_search
    ],
    system_prompt="""
        You are a travel advisor.
        You can tell the current time in seconds,
        or get current month,
        look up countries by name,
        look up places to visit and recommend the best time to visit.
    """
)
...
def run_agent(user_input) -> str:
    ...
    try:
        ...
        if str(type(response)).find('starlette') != -1:
            ...
            logger.warning(
                """
                Starlette response detected but couldn't extract content: %s
                """,
                type(response)
            )
            return """
                    I apologize, but I'm experiencing a technical issue with the response format.
                    Please try again.
                    """
